...
class IRISClassification():
    ...
    def create_pipeline(self):
        ...
        model_path = os.path.join(model_folder, self.args.model_filename)
        joblib.dump(model, model_path)

        print("Uploading the model into run artifacts...")
        self.run.upload_file(name="./outputs/models/" + self.args.model_filename, path_or_stream=model_path)
        print("Uploaded the model {} to experiment {}".format(self.args.model_filename, self.run.experiment.name))
        dirpath = os.getcwd()
        print(dirpath)
        print("Following files are uploaded ")
        print(self.run.get_file_names())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_filename', type=str, help='Name of the file containing the model')
